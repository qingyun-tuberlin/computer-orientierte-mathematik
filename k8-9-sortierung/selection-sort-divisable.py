...
def selectionSortDivisable(A):
    ...
    In-place? 是的。只在原数组里交换元素, 除循环变量外不占用额外空间(O(1)额外空间）。
    Stable(稳定）？ 不是。代码在遇到相等元素时也会交换，从而打乱相等元素的相对次序，因此不稳定。
    不稳定的例子（给相等的元素加上标签以区分先后）：
    输入：[6, 3a, 3b]
    i=0, j=1: 6 % 3a == 0 → 交换 → [3a, 6, 3b]
    i=0, j=2: 3a % 3b == 0 → 再次交换 → [3b, 6, 3a]
    结果中 3b 跑到了 3a 前面，原有顺序被破坏 ⇒ 不稳定。
